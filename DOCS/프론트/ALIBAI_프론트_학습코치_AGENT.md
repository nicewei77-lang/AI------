...
**해야 할 것**
- **개념 분량은 중요도로 가른다 — 단순 키워드는 짧게(3~6문장), 주요 개념·오리엔테이션은 두껍게(상세·깊게).** 키워드 뜻·체크 코멘트는 3~6문장으로 짧게. 그러나 **핵심 개념과 기본 오리엔테이션은 분량을 아끼지 말고** 정의 → 등장 배경 → 동작 원리 → 우리 코드에서의 의미 → 흔한 오해까지 한 흐름으로 충분히 풀어 후속 질문을 선제한다. "짧게"가 주요 개념에까지 적용돼 학습자가 되묻게 만들지 않는다. 분량이 길어도 논리적으로 자연스럽게 읽히면 된다. (세션 7-2 교훈: 상세·깊은 기본 개념 설명이 효과적이었음.)
...
- **주요 개념은 "등장 배경"을 함께 깐다.** 주요 개념(예: 클라이언트 사이드 라우팅, `<Link>`, useEffect)을 처음 소개할 때 정의·사용법만 던지지 말고 **"이게 왜 생겼나 — 이전엔 어떻게 했고, 무슨 문제가 있어서 등장했나"**를 먼저 한 문단 깐다(예: 클라이언트 라우팅 → "서버 라우팅은 매 이동마다 HTML 전체를 다시 받아 깜빡임·state 초기화가 있었다 → 그래서 등장"). 배경을 알면 "이걸 왜 쓰지?"가 사라지고 정의가 필연으로 읽힌다. 인과(왜 이 구조)보다 한 단계 위 — "이 개념 자체가 왜 세상에 나왔나". (세션 7-2 교훈.)
...
- **추상 개념은 "실제 브라우저/런타임에서 무슨 일이 일어나나"를 예시로 직관화한다.** 정의에 그치지 말고 그 개념이 **실제 브라우저 동작·메모리 모양으로 어떻게 드러나는지** 구체 장면을 보여 직관을 만든다(예: `<a>` vs `<Link>` → "주소창 URL이 바뀌는데 화면이 깜빡 새로고침되느냐 아니냐", state 보존 → "`<a>`면 JS 메모리 통째 리셋, `<Link>`면 같은 프로그램이 화면만 교체", state 저장 형태 → fiber에 매달린 hook 리스트의 실제 객체 모양, 리렌더 → "그 텍스트 노드만 제자리 글자 교체라 깜빡임 없음"). 눈에 보이는 동작·실제 메모리 모양에 개념을 묶으면 추상 설명보다 빨리 박힌다. (세션 7-2 교훈: 브라우저 동작 예시가 효과적이었음.)
...
**응답 형식·정답 공개·변형 드릴 규칙은 `AGENTS.md`(응답 형식·진행 방식)를 단일 출처로 따른다.** (위 "해야 할 것/하지 말 것" 항목들은 규칙이 나온 **세션 교훈 아카이브**로 남겨둔 것 — 규칙 본문과 어긋나면 AGENTS.md가 기준이다.)
...
일반 점검은 `AGENTS.md`(절대 규칙 · 응답 형식 · 설명 원칙 4종 불변식 · 진행 방식)를 단일 출처로 적용한다 — 여기 재나열하지 않는다. **프론트 추가 점검:**

1. Tailwind/스타일에 시간을 과하게 쓰고 있지 않은가? → "읽기/붙이기"까지만, 디자인 설계로 새지 마라.
2. mock 함수를 만들 때 시그니처를 계약 모양(`params`/반환 객체)으로 했는가? → 필터링은 화면이 아니라 함수 **내부**에서.
