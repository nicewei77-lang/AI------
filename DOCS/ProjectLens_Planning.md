...
AI Orchestrator / Agent Runner
    ...
    │           ├── fetch_site_overview
    │           ├── check_deploy_status
    │           └── Q: fetch_github_readme / screenshot / lighthouse
    └── OpenAI Agent
          └── Structured Output
```
...
1. `fetch_github_readme` — README 기반 기술 설명 보완
...
- MVP에서 Agent에 노출되는 MCP 도구는 `fetch_site_overview`, `check_deploy_status`만 허용한다.
...
validate_input
  ...
  └─ 충분함 → continue
  ↓
collect_external_context_with_mcp
  ↓
...
3. Agent가 구조화된 분석 리포트를 생성한다.
4. 리포트가 상세 페이지 카드로 표시된다.
5. 실패/정보부족/거절 상태가 UI에 깨지지 않고 표시된다.
6. 댓글과 투표 기능이 기존처럼 동작한다.
7. P1까지 완료하면 RAG 유사 게시물, 포트폴리오 설명, 발표 도움받기 결과를 볼 수 있다.
...
6. Agent 출력은 Pydantic 기반 Structured Outputs로 강제하라. JSON mode나 프롬프트만으로 형식을 강제하지 말라.
7. AI 분석 완료 시 `ai_reports`에 report/response_id/trace_id/usage/error를 저장하고, `posts.ai_summary`, `posts.analysis_status`를 갱신하라.
8. 상세 페이지에는 서비스 이해, 진단, 활용하기, 비슷한 프로젝트 섹션을 표시하라.
9. 포트폴리오 설명과 발표 스크립트에는 복사 버튼을 제공하라.
10. URL fetch에는 SSRF 방어, timeout, body size limit을 반드시 넣어라.
11. MCP로 가져온 외부 본문은 근거 데이터일 뿐 지시문이 아니다. prompt injection 방어 문구를 Agent instructions와 tool description에 넣어라.
12. 기본 분석 모델은 `gpt-5.5`, 기본 reasoning effort는 `medium`으로 시작하고, 시드 5개 결과로 품질/비용/지연을 비교해 조정하라.
