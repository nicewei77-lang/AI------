# ProjectLens AI 파트 코어타임 설명초안

> 긴 워크숍형 MD 초안 v3.  
> 목적은 ProjectLens 서비스를 소개하는 것이 아니라, 과제의 AI 필수 요구사항인 RAG, MCP, AI Agent를 이해시키고 구현 판단 기준을 잡아주는 것이다.  
...
ProjectLens는 이 세 개념을 보여주는 예시일 뿐이다. 발표의 중심은 "내 서비스 설명"이 아니라 "과제에서 요구하는 AI 활용 기능을 어떻게 이해하고 구현할 것인가"다.
...
- 구현 표와 코드 snippet은 발표 중 전부 읽지 않는다.
- Notion 이미지는 만료 URL을 넣지 않고, PPT 제작 시 다시 가져올 이미지 자리만 표시한다.
...
PPT 사용 의도: "학습이 아니라 검색해서 읽게 한다"를 한 장으로 보여주기
```
...
PPT 사용 의도: "MCP는 AI 앱의 공통 연결 포트"라는 감각을 잡기

이미지 자리 2: MCP Host / Client / Server 구조
출처: Notion [JUNGLE] AI로 진화하기 / MCP
PPT 사용 의도: LLM, Host, Client, Server의 역할을 분리해서 보여주기
```
...
PPT 사용 의도: Agent가 모델 하나가 아니라 실행 구조라는 점 보여주기

이미지 자리 2: ReAct Think -> Act -> Observe 루프
출처: Notion [JUNGLE] AI로 진화하기 / AI agents
PPT 사용 의도: Agent 루프가 발표의 핵심임을 시각화하기
```
...
## 3. 발표용 핵심 요약: RAG/MCP/Agent 각각 3문장
...
발표에서는 이 표를 전부 읽지 말고, "우리 구현에서는 이 루프를 게시글 분석 기능에 붙였다" 정도로 말한다.

---

## 5. RAG 구현 가이드: 발표 핵심 + 구현 참고표
...
다만 발표에서는 "데이터가 충분히 쌓이면 의미 유사도 외에도 태그/유형/최신성 같은 신호를 섞을 수 있다" 정도만 말한다.
...
발표에서는 "Agent는 실패와 지연까지 상태로 다뤄야 서비스가 된다" 정도로 말한다.
...
발표에서는 OpenAI API를 쓸 때 비용과 제한을 어떻게 조심해야 하는지만 말한다.
...
## 12. Canva PPT outline + 이미지 캡션 목록
...
| 2 | 오늘의 목표 | 발표는 핵심, 세부는 구현 참고로 분리한다. |
...
### 12.3 Canva prompt로 넘길 재료

```text
목적: 과제의 AI 필수 요구인 RAG/MCP/Agent를 이해시키는 발표자료
...
주의: Notion signed image URL 직접 사용 금지, Canva 제작 시점에 이미지 재수집
```
...
1. 발표 시간이 10분이면 구현 참고표는 거의 넘기고 핵심/루프/비용만 말한다.
2. 발표 시간이 20분이면 RAG/MCP/Agent 구현 참고표를 각각 한 장씩 넣는다.
3. Canva 제작 직전에 Notion 이미지를 다시 가져와 만료되지 않는 방식으로 배치한다.
