...
- tool allowlist: P0에서 Agent가 호출 가능한 MCP 도구는 `fetch_site_overview`, `check_deploy_status`만. `fetch_github_readme`는 Q5.
...
- `backend/app/ai/agents/project_analysis_agent.py` + `ai/runner.py`: `Agent(..., model=settings.agent_model, output_type=ProjectAnalysisReport)` 형태로 Pydantic Structured Outputs 사용. 기본 모델 `gpt-5.5`, 기본 `reasoning.effort=medium`.
- `ai/runner.py` 흐름 = load_post_context → validate_input(불충분 시 `need_more_info`) → collect_external(MCP) → evidence 정제/길이 제한 → (RAG는 M4 합류) → structured output → save.
...
### M4 · RAG 유사글 (정직하게 작게) — [P1]

**목표:** 게시판에 쌓인 프로젝트를 검색 가능한 판례집으로. **데이터 적을 땐 약함을 인정**한다.
...
- 합류: top-k를 Agent 입력의 `similar_projects` 근거로 + 프론트 `SimilarProjectsCard`.
- 빈 상태 정직 처리: 유사도 임계값 미만 → "비슷한 게시물이 아직 충분하지 않습니다".

**검증:** 시드 5개 임베딩 저장 후 한 글에서 유사글이 합리적 이웃 반환, 임계값 미만 시 빈 메시지. Agent 리포트의 `evidence.rag_sources`에 출처 기록.
...
- **Q1 · 실패 모드 견고화 [최고 ROI]:** 기획서 §20의 5개 — URL 실패 graceful 안내, 사이트 텍스트 빈약 → `body` 폴백(Q5에서 GitHub README), RAG 빈 결과, Structured Output validation/refusal 처리, 느린 분석 → loading. *"실제 쓸 만한 퀄리티"를 죽이는 건 AI 성능이 아니라 통합 글루·엣지케이스다.*
...
- **Q5 · MCP 확장(선택):** `fetch_github_readme`(README 기반 기술 추출), `check_deploy_status` 결과 노출, 그 외 §9.4 후보.
...
Q1 [ ] 실패 모드 5종  Q2 [ ] 프롬프트/스키마  Q3 [ ] 포폴/발표  Q4 [ ] RAG 가중  Q5 [ ] MCP 확장(fetch_github_readme 등)
```
