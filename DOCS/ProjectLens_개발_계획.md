...
- DB `backend/db/schema.sql`:
  ...
  ```
  > `analysis_jobs` 테이블은 아직 컷. Q11 async polling은 `posts.analysis_status`로 먼저 구현한다.
...
- 라우터 `backend/app/routers/analysis.py`: `POST /posts/{id}/analysis`(동기 smoke/debug용), `POST /posts/{id}/analysis/jobs`(Q11 async start), `GET /posts/{id}/analysis/status`, `GET /posts/{id}/analysis/latest`. `backend/app/main.py`에 `analysis_router` 등록.
...
멀티 에이전트 분리 · 별도 `analysis_jobs` 테이블/외부 큐 · 아이디어 게시판(기획서도 "선택") · 전체 재생성 버튼 · 웹 전체 크롤링 · robots/broken links 전체 점검 · GitHub Issue 자동 생성 · Slack/Notion 연동.
...
- 분석 = **async polling**(Q11). 별도 `analysis_jobs` 테이블은 아직 만들지 않고 `posts.analysis_status`를 durable 상태로 쓴다. RAG = **weighted**(Q4).
...
```text
M0 [x] posts 클린 마이그레이션 + ai_reports/mcp_evidences/embeddings + CREATE EXTENSION vector
   [x] CHECK 제약 + response_id/trace_id/usage/error 컬럼
   [x] models/schemas/repo/service 갱신, requirements 4종, config+.env(gpt-5.5/medium)
M1 [x] mcp-server fetch_site_overview + check_deploy_status + SSRF safety
   [x] local/private backend mcp_client 연결 + tool allowlist + prompt injection 방어
M2 [x] ProjectAnalysisReport(트림) + prompts + agent/runner + analysis_service
   [x] POST /posts/{id}/analysis + Structured Outputs + response/usage/error 저장
   [x] need_more_info/failed/refused 검증
M3 [x] types/api/analysis + P0 카드 4종 + ProjectForm + PostDetail/Card 개편 → 브라우저 E2E
M4 [x] embedder/indexer/retriever(cosine) + SimilarProjectsCard + 빈상태
