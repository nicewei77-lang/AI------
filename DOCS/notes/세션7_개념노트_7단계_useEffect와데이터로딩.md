...
왜 이름이 "배열"이냐면, React가 **한 개가 아니라 여러 의존값을 목록으로 받기 때문**이다.
...
이건 "`id`나 `q`나 `tagId` 중 하나라도 바뀌면 다시 실행"이라는 뜻이다.  
즉 의존성 배열은 "재실행 버튼 목록" 정도로 이해하면 된다. 지금 `[]`는 그 목록이 **비어 있는 상태**다.
...
이건 **JavaScript 문법**이고, 보통 **화살표 함수(arrow function)** 라고 부른다. 뜻은 "함수를 조금 더 짧게 쓰는 방법" 정도로 잡으면 된다.
...
- `()` = 이 함수가 받는 매개변수 자리(지금은 없음)
- `=>` = "이제 함수 본문이 나온다"는 화살표
- `{}` = 실행할 코드 묶음

지금 코드의 `useEffect(() => { ... }, [])`에서 이 화살표 함수는 **React에게 넘기는 콜백**이다. 즉 "화면이 그려진 뒤 이 코드를 실행해 줘"라고 맡기는 함수.

어디서 많이 보이냐면:
- `useEffect(() => { ... })` 같은 Hook 콜백 자리
- `onClick={() => { ... }}` 같은 이벤트 핸들러 자리
- `posts.map(post => ...)` 같은 배열 메서드 콜백 자리

`function`과의 차이는 지금 단계에서는 이 정도만 잡으면 충분하다:
- `function hello() {}` = 이름을 붙여 따로 선언하는 느낌
- `() => {}` = 다른 함수에 **인자로 넘길 짧은 함수**를 쓸 때 자주 쓰는 표현
- 화살표 함수는 자기만의 `this`를 새로 만들지 않지만, 지금 단계에서는 이 차이보다 **"콜백 자리에서 자주 쓴다"** 가 더 중요하다

한 가지 더 보면:
- `function hello() {}`는 **함수 선언문**
- `const hello = () => {}`는 **함수 표현식을 변수에 담은 것**

그래서 읽는 느낌도 조금 다르다:
- 선언문 = "이름 있는 함수 하나를 따로 만든다"
- 화살표 함수 = "여기 이 자리에서 바로 쓸 함수를 만든다"

`useEffect(() => { ... })`에서 화살표 함수를 쓰는 이유도 이쪽이다. **딱 그 자리에서 React에게 넘길 콜백**이 필요해서다.

### 6-2. `useEffect`와 `setPosts`는 뭐가 다른가? (드디어 풀린 지점)

처음엔 둘 다 "나중에 뭔가 예약"처럼 보여서 헷갈린다. 하지만 **예약하는 대상이 완전히 다르다.**

| 코드 | 정확한 역할 | 무엇을 예약/등록하나 |
|---|---|---|
| `useEffect(() => { ... }, [])` | 렌더가 끝난 뒤 실행할 작업 등록 | **effect 콜백 실행**을 등록 |
| `setPosts(data.items)` | state 변경 요청 | **다음 렌더**를 예약 |
| `fetchPosts()` | 데이터 가져오기 | 렌더 예약 아님. 데이터만 가져옴 |
| `return <...>` | 이번 렌더의 화면 설계도 반환 | 지금 state 기준 JSX를 React에게 줌 |

즉:
- `useEffect` = "렌더 뒤에 이 일을 시작해 줘"
- `setPosts` = "state가 바뀌었으니 다시 렌더해 줘"

`useEffect` 자체가 렌더를 예약하는 게 아니다. **렌더를 예약하는 건 setter(`setPosts`, `setLoading`)다.**

#### `useEffect` 줄을 만나면 바로 실행되는가?
...
- `useEffect(...)` **호출 자체**는 렌더 중에 지나간다.
- 하지만 `() => { ... }` **콜백 안쪽 본문**은 그 자리에서 실행되지 않는다.
- React는 콜백을 내부 목록에 적어둔다.
- 나중에 DOM 반영(commit/patch)이 끝난 뒤 그 콜백을 실행한다.
...
`addEventListener` 줄을 만났다고 `clicked`가 바로 찍히지 않는다. "클릭하면 이 함수 실행"이라고 등록만 하고 지나간다.  
`useEffect`도 "렌더와 DOM 반영이 끝나면 이 함수 실행"이라고 등록만 하고 지나간다.

#### 그럼 정확히 뭐가 완료되면 `load()`가 실행되나?

**첫 렌더 결과가 실제 DOM에 반영되는 commit/patch가 끝나면** `useEffect` 콜백이 실행되고, 그 안에서 `load()`가 호출된다.
...
```tsx
<p className="p-8">불러오는 중...</p>
```

React가 이 결과를 실제 브라우저 DOM, 보통 `index.html`의 `<div id="root">` 아래에 반영한다.

```html
<div id="root">
  <p class="p-8">불러오는 중...</p>
</div>
```

여기까지가 **첫 DOM 패치**다. 즉 맨 처음 DOM 패치의 영역은 "React가 관리하는 root 아래에 로딩 화면을 넣는 것"이다.

그 다음에야 React가 아까 등록해 둔 effect 콜백을 실행한다.
...
1. 렌더 phase
   ...
   useState 값 읽음
   useEffect 콜백 등록
   loading=true라서 로딩 JSX return

2. commit/patch phase
   return된 로딩 JSX를 실제 DOM에 반영
   #root 아래에 <p>불러오는 중...</p>가 들어감

3. effect 실행
   등록해 둔 useEffect 콜백 실행
   콜백 안에서 load() 실행
   load 안에서 fetchPosts() 시작

4. fetch 완료 후
   setPosts(data.items)
   setLoading(false)
   이 setter들이 다음 렌더를 예약

5. 두 번째 렌더 + 두 번째 DOM 패치
   loading=false, posts=[실제 글]
   로딩 <p>를 목록 화면 DOM으로 교체
```

핵심 문장:

> `useEffect`는 **첫 화면을 그리는 함수**가 아니라, **첫 화면이 실제 DOM에 반영된 뒤 실행할 일을 등록하는 Hook**이다.

그리고:

> `setPosts`는 **그 일을 하다가 데이터가 도착했을 때 state를 바꾸고 다음 렌더를 예약하는 setter**다.
...
### 8-5-1. 함수 정의와 호출이 같은 블록에 같이 있어도 되나? (질문)

된다. **함수 정의**와 **함수 호출**은 서로 다른 문장이라 같은 블록 안에 나란히 있어도 전혀 이상하지 않다.

```ts
useEffect(() => {
    async function load() {   // 정의: "이런 일을 하는 함수를 만든다"
        await fetchPosts();
    }

    load();                   // 호출: "방금 만든 함수를 실제로 실행한다"
}, []);
```
...
여기서는 `load`를 만든 이유가 "한 번 쓰고 버릴 이름 붙은 헬퍼"가 필요해서가 아니라, **`await`를 둘 async 울타리**가 필요해서다. 그래서 같은 블록 안에 `load`를 정의하고 바로 `load()`로 호출하는 패턴이 자연스럽다.
...
여기서 `loading`이 답하는 질문은 **"지금 기다리는 중인가?"** 하나뿐이다.  
성공이면 더 이상 기다리는 중이 아니고, 실패여도 더 이상 기다리는 중이 아니다. 둘 다 **대기 종료**이므로 `finally`에서 `setLoading(false)`가 맞다.

헷갈리기 쉬운 포인트: `loading=false`는 **"성공했다"** 가 아니다. 뜻은 정확히 **"로딩이 끝났다"** 다.  
성공 여부는 `posts`, 실패 여부는 `error`, 진행 중 여부는 `loading`이 따로 맡는다.
...
### 11-1. 그럼 화면 업데이트는 누가 하나? (질문)

지금 단계에서는 이렇게 딱 나누면 된다:
- 우리는 **state를 setter로 바꾼다**
- React는 그걸 보고 **컴포넌트를 다시 실행해서 화면을 갱신한다**

즉 `setPosts(data.items)`를 호출한 뒤 우리가 DOM을 직접 만지는 코드는 안 쓴다. React가 알아서:
1. state 변경을 기록하고
2. `PostListPage()`를 다시 호출하고
3. 새 JSX를 계산한 뒤
4. 이전 화면과 비교해서 **바뀐 DOM만** 고친다

여기서 층위를 구분하는 게 중요하다:
- **state 쪽:** `posts` 칸의 값은 새 배열로 **교체**
- **화면(DOM) 쪽:** React가 비교 후 **바뀐 부분만 갱신**

그래서 "바뀐 값만 반영"이라는 말은 **DOM 업데이트 층위에서는 맞고**, `useState`의 `posts` 칸 자체는 **새 값으로 교체**된다고 이해하면 가장 덜 헷갈린다.
