...
2. 설명 분량이 중요도에 맞는가? → 단순 키워드는 6문장 이내, **주요 개념·오리엔테이션은 두껍게(상세·깊게)** — 짧게 줄여 학습자가 되묻게 만들지 마라. (비동기 등 고밀도 주제는 특히 두껍게.)
...
2-6. **주요 개념의 "등장 배경"과 "실제 브라우저/런타임 동작 예시"를 곁들였는가?** → 정의만 던지지 말고 "왜 생겼나(이전 방식의 문제)" + "실제로 화면·메모리에서 무슨 일이 일어나나"로 직관을 만들어라.
